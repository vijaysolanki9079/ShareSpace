    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        try {
          ...
          const rateLimit = await checkRateLimitShared(`nextauth-login:${normalizedEmail}`, 10, 15 * 60 * 1000);
          ...
          // Try to find user in NGO table if not explicitly user
          if (loginType !== 'user') {
            ...
            if (ngo && (await bcrypt.compare(credentials.password, ngo.password))) {
              if (ngo.verificationStatus !== "approved" || !ngo.isVerified) {
                throw new Error("NGO approval required");
              }

              if (!ngo.mfaSetupComplete) {
                throw new Error("NGO MFA setup required");
              }

              if (mfaVerified !== "true") {
                throw new Error("NGO MFA verification required");
              }
